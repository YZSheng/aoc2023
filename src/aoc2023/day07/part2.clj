...
(defn calculate-hand-strength [hand]
  (-> hand
      (str/split #"")
      frequencies
      vals
...
(defn map-j [card]
  (let [freq (frequencies (str/split card #""))]
    (if (nil? (freq "J"))
      card
      (str/replace card "J" (or (->> freq
                                     (filter #(not= "J" (first %)))
                                     (sort-by (juxt val (comp card-to-number first)))
                                     last
                                     first) "A")))))

(defn calculate-hand-score [hand]
  (+ (calculate-hand-strength (map-j hand)) (calculate-cards-value hand)))
...
(part-2 "resources/day07/sample.txt")
(part-2 "resources/day07/input.txt")